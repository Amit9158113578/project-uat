...
export class EncryptionUtil {
    ...
    public static encryptString(plainText: string): string {
        const cipher = crypto.createCipher('aes256', CommonConstants.JWTPRIVATEKEY);
     ...
     */
    public static decryptString(encryptedText: string): string {
        const decipher = crypto.createDecipher('aes256', CommonConstants.JWTPRIVATEKEY);
     ...
     * @returns decrypted string
     */
    public static comparePasswords(plainTextPassword: string, encryptedPassword: string): boolean {
 ...
 * @description Request to be signed
